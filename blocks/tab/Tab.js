export class Tab {
    ...
    linkClickHandler(event) {
        if (this.params._active && !this.params._animationInProgress) {
            ...
            if (!link.classList.contains(this.classes.linkActive)) {
                const activeLink = this.tab.querySelector(`.${this.classes.linkActive}`)
                const currentPanel = this.panelContainer.querySelector(`${this.selectors.panel}.${this.classes.panelActive}`)
                const nextPanel = this.panelContainer.querySelector(`${this.selectors.panel}[data-tab="${linkTabID}"]`)
                ...
                if (nextPanel) {
                    ...
                    activeLink.classList.remove(this.classes.linkActive)
                    link.classList.add(this.classes.linkActive)

                    if (this.params.handleURI) {
                        this.changeURIHash(linkTabID);
                    }
                }
